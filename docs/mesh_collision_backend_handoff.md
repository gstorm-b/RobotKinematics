...
- `collision_profiles/nachi_mz04d_mesh_collision.json` covering all eight Nachi MZ04D STL parts
...
5. `docs/collision_detection_plan.md`
6. `docs/mesh_collision_backend_plan.md`
7. `docs/developer_onboarding.md`
8. `AGENTS.md`

## First Task

Start by reading [docs/mesh_collision_backend_spike.md](/D:/Project/RobotKinematics/docs/mesh_collision_backend_spike.md).
...
- Do not commit local optional dependency checkouts or installs from `third_party`; they are ignored
  development artifacts except for tracked `third_party/eigen`.

## Dependency Spike Output Template

`docs/mesh_collision_backend_spike.md` now exists and should be kept current with any new Coal/FCL
...
These directories are local development artifacts and should not be staged. The committed contract is
the optional qmake integration plus reproducible scripts; dependency source/install trees stay out of
git unless the project manager explicitly changes packaging policy.
...
`docs/mesh_collision_backend_spike.md`; keep final distribution/legal approval as a release
...
- delivered in this slice: Phase 10.6 Nachi mesh profile + tests; Phase 10.7 simplification tool
  ...
  `docs/mesh_collision_backend_plan.md`.
