...
- Phase 10.6 is implemented: `collision_profiles/nachi_mz04d_mesh_collision.json` carries all
...
- Phase 10.7 ships `tools/mesh_simplification/` and the convenience scripts
  ...
  `collision_profiles/simplified/` and `collision_profiles/*_simplified*.json`.
...
- Review fixes before Phase 10.6 are complete: relative mesh paths loaded from profile files are
  ...
  safety-margin behavior is covered by test, and optional third-party dependency checkouts/install
  roots are ignored by git.
...
Local dependency checkout/install directories used by the helper scripts are development artifacts:
`third_party/assimp`, `third_party/boost`, `third_party/coal`, `third_party/fcl`,
`third_party/libccd`, and `third_party/install` are ignored by git. Keep Eigen as the only tracked
vendored dependency unless the user explicitly approves a different packaging policy.
